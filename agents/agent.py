...
class DDPG():
    ...
    def __init__(self, task):
        ...
        self.action_low_all = task.action_low_all
        self.action_high_all = task.action_high_all

        # Actor (Policy) Model
        self.actor_local = Actor(self.state_size, self.action_size, self.action_low, self.action_high, self.action_low_all, self.action_high_all)
        self.actor_target = Actor(self.state_size, self.action_size, self.action_low, self.action_high, self.action_low_all, self.action_high_all)
        ...
        self.exploration_sigma = np.array([0.01,0.01,0.01,0.01,1.])
        ...
        self.tau = 0.0001 # 0.001 was best for contrained speeds 0.01  # for soft update of target parameters
    ...
    def step(self, action, reward, next_state, done):
        ...
        if len(self.memory) > 2*self.batch_size:
            ...
            experiences_actor = self.memory.sample()
    ...
    def learn_actor(self, experiences):
        ...
        # actions is from experiences, but shouldn't we take the actions from the current actor?
        # also, maybe independent sample for the actor?
        actions_current = self.actor_local.model.predict_on_batch(states)
        action_gradients = np.reshape(self.critic_target.get_action_gradients([states, actions_current, 0]), (-1, self.action_size))
        self.actor_local.train_fn([states, action_gradients, 1])  # custom training function
